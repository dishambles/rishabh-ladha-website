...
Form is a plain HTML POST (no JS submission handling) in `#contact`, styled in-system, with a `_honey` honeypot, wired to FormSubmit.co with Rishabh's email: **rishabhladha17@gmail.com**. **One-time activation:** a test submission was sent from the live contact form on 2026-09-26 (owner-approved); FormSubmit replied "Check Your Email" and emailed an 'Activate Form' link to rishabhladha17@gmail.com. **Activated** — owner confirmed 2026-09-26; submissions now deliver to rishabhladha17@gmail.com. Both forms (contact + resources request-access) post to the same address. Re-activated for the new domain on 2026-09-26 (FormSubmit treats each domain as a new form). FormSubmit shows its own "I'm not a robot" checkbox page after Send (kept on purpose; owner chose not to disable it). Each form has a hidden `_next` field so FormSubmit returns people to the site: `/?sent=1#contact` and `/resources/?requested=1#request`; `motion.js` then swaps the fields for the form's `.form__sent` note and strips the flag from the URL. Direct channels shown alongside: email (mailto), LinkedIn (`in.linkedin.com/in/rishabhladha`), X (`@rishabhladha`).
